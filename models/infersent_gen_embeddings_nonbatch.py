...
print("Text Shortening Complete!")
...
text_full = df_new['text_full_limited']

#writing function for common cosine similarity
def doc_words_cosine(i, nterms):
    emb = embeddings[i]
    if nterms == 5: #seed
...
#defining the vocabulary for inquiry-based learning; #words: 5, 30, 100, 500.
...
print("Inquiry terms' embeddings generated!")

#generating embeddings
embeddings = model.encode(text_full, verbose=True)
print('schools encoded : {0}'.format(len(embeddings)))

#initializing list for appending cosine values
seed_ls = []
for i in range(len(df_new)):
    seed_ls.append(doc_words_cosine(i, 5))
thirty_ls = []
for i in range(len(df_new)):
    thirty_ls.append(doc_words_cosine(i, 30))
hundred_ls = []
for i in range(len(df_new)):
    hundred_ls.append(doc_words_cosine(i, 100))
fivehundred_ls = []
for i in range(len(df_new)):
    fivehundred_ls.append(doc_words_cosine(i, 500))
    
    
df_new['infersent_inqseed_cosine'] = seed_ls
df_new['infersent_inq30_cosine'] = thirty_ls
df_new['infersent_inq100_cosine'] = hundred_ls
df_new['infersent_inq500_cosine'] = fivehundred_ls

df_new.to_csv("df_with_cosine_scores_infersent.csv")
